...
<form method="post" action="/teacher/students/import" class="space-y-2"><input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>"><input type="hidden" name="team_id" value="<?= $teamId ?>"><textarea name="csv_data" class="border p-2 w-full h-32" placeholder="Nombre,correo@x.com"></textarea><button class="bg-green-600 text-white px-3 py-1">Importar CSV pegado</button></form>
</section>
